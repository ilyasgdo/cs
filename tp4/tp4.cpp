...
float g_CamRadius = 15.0f;
float g_CamTheta = 0.0f;
float g_CamPhi = 1.57f;
double g_LastMouseX = 0.0;
double g_LastMouseY = 0.0;
bool g_FirstMouse = true;

void OnCursorPos(GLFWwindow* window, double xpos, double ypos) {
    if (g_FirstMouse) { g_LastMouseX = xpos; g_LastMouseY = ypos; g_FirstMouse = false; return; }
    double dx = xpos - g_LastMouseX;
    double dy = ypos - g_LastMouseY;
    g_LastMouseX = xpos;
    g_LastMouseY = ypos;
    g_CamPhi -= dx * 0.005f;
    g_CamTheta += dy * 0.005f;
    if(g_CamTheta > 1.57f) g_CamTheta = 1.57f;
    if(g_CamTheta < -1.57f) g_CamTheta = -1.57f;
}

void OnScroll(GLFWwindow* window, double xoffset, double yoffset) {
    g_CamRadius -= yoffset;
    if(g_CamRadius < 1.0f) g_CamRadius = 1.0f;
}

void OnWindowSize(GLFWwindow*, int w, int h) { g_WindowWidth = w; g_WindowHeight = h; }
void OnFramebufferSize(GLFWwindow*, int fbw, int fbh) { glViewport(0, 0, fbw, fbh); }
...
void MatrixLookAt(float* m, const float* eye, const float* target, const float* up) {
...
void Render(float time) {
    ...
    float eye[3] = { g_CamRadius * cos(g_CamTheta) * cos(g_CamPhi), g_CamRadius * sin(g_CamTheta), g_CamRadius * cos(g_CamTheta) * sin(g_CamPhi) };
    float target[3] = { 0.0f, 0.0f, 0.0f };
    float up[3] = { 0.0f, 1.0f, 0.0f };
    
    float view[16], proj[16];
    MatrixLookAt(view, eye, target, up);
    MatrixPerspective(proj, 45.0f, (float)g_WindowWidth/g_WindowHeight, 0.1f, 100.0f);
    ...
    glUniform3f(glGetUniformLocation(prog, "u_CameraPos"), eye[0], eye[1], eye[2]);
    ...
    MatrixTranslation(tC, -4.0f, 0.0f, 0.0f);
